...
const BossCard: React.FC<BossCardProps> = ({ boss }) => {
  ...
  return (
    <div className="w-[450px] h-[180px] bg-white/5 rounded-sm relative mt-12">
      <img
        src={`/images/bosses/${boss.name.toLowerCase().replaceAll(" ", "-")}.png`}
        className="absolute max-w-[200px] -top-8 -left-20"
        alt={boss.name}
      />
      <div className="ml-[7rem] text-center relative top-3">{boss.name}</div>
      ...
      <div className="ml-[7rem] flex flex-col gap-2 relative top-3 mt-2">
        ...
        <div className="flex items-center text-sm">
          ...
          <div className="h-[5px] w-[calc(100%-70px)] ml-2 bg-white/20 rounded-sm">
            <div
