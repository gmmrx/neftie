...
const LatestHatchRow: FC<LatestHatchRowProps> = ({ hatch }) => {
  const stars = ["Common", "Uncommon", "Rare", "Epic", "Legendary"];
  const starCount = stars.indexOf(hatch.neftie_rarity) + 1;
      ...
      <div className="flex flex-col">
        ...
        <div className="flex items-center font-inter text-[0.625rem] text-[#768192] gap-[1rem] mt-1">
          <div className="flex items-center gap-[3px]">
            <EggIcon size={10} /> {hatch.egg_name}
          </div>
          <div className="flex items-center gap-[3px]">
            <ClockIcon size={10} /> {moment(hatch.createdAt).fromNow()}
          </div>
        </div>
      </div>
    </div>
